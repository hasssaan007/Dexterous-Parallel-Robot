...
writerObj = VideoWriter('hexapo-sim3.avi');
...
for k = 1:length(z);
    ...
    for j = 1:6
        % Legs
        plot3([b(1,j),q(1,j)],[b(2,j),q(2,j)],[b(3,j),q(3,j)],'k','LineWidth',1.5);
        
        % Servo arms & legs
        plot3([b(1,j),xa(j)],[b(2,j),ya(j)],[b(3,j),za(j)],'b','LineWidth',3);
        plot3([q(1,j),xa(j)],[q(2,j),ya(j)],[q(3,j),za(j)],'b','LineWidth',3);
    end
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ijb=inverse_jacob_hexapod3(A1,A2,A3,A4,A5,A6,A7,B1,B2,B3,B4,B5,B6,B7);
    %ijbvect(:,:,i)=ijb;
    condnumber3(k)=cond(ijb)
   ...
   %plot5=subplot(3,2,[6]); %% Condition Number
   %%
   %hold on
    xlabel('x-axis')
    ylabel('y-axis')
...
% Inverse conddition number
condumber3inv=1./condnumber3;
figure
plot(condumber3inv)
...
plot(condnumber3)
hold on
legend({'normal','1 redundancy', 'Full redundancy'},'Location','southwest')
